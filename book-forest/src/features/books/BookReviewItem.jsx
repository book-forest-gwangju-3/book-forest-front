...
const BookReviewItem = ({ item, token, onCommentUpdate, onCommentDelete }) => {
  ...
  const handleSave = async () => {
    try {
      const response = await axios.patch(
        ...
        { quickReviewId: item.quickReviewId, content: editedContent },
        {
          headers: {
  ...
  const handleDelete = async () => {
    if (window.confirm("정말로 이 댓글을 삭제하시겠습니까?")) {
      try {
        await axios.delete("http://localhost:8080/books/quick-reviews", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
          data: { quickReviewId: item.quickReviewId },
        });
        onCommentDelete(item.quickReviewId); // 부모 컴포넌트에 삭제된 댓글 id 전달(Optimistic UI Update)
      } catch (error) {
